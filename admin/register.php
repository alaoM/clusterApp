...
if (empty($_POST['clusterCode'])) {
  $error[] = "Please enter your Cluster Code";

}
else{
   $stmt = $db->prepare('SELECT clusterCode FROM chairman WHERE clusterCode = :clusterCode');
  $stmt->execute(array(':clusterCode' => $_POST['clusterCode']));
  $row = $stmt->fetch(PDO::FETCH_ASSOC);

  if ($_POST['clusterCode'] == $row['clusterCode']) {
    $error[] = "Cluster Code already in Use";
  }
  ...
  if (!isset($error)) {
    ...
    try {
      ...
      $stmt = $db->prepare('INSERT INTO chairman (clusterCode,State,fullName,password,email,phoneNumber) VALUES (:clusterCode,:State,:fullName,:password,:email,:phoneNumber)');
      $stmt->execute(array(
            ...
            <div>
              ...
              <form name="RegisterForm" id="RegisterForm" target="_blank" role="form" action="" method="post" autocomplete="off">
                ...
                <div class="register_content text-center"><i class="glyphicon glyphicon-envelope"></i></span>
                  <input type="text" name="clusterCode" id="email" placeholder="Cluster Code" class="input_field search_form_name" maxlength="100" value="<?php if (isset($error)) {
                                                                                                                                                            echo $_POST['clusterCode'];
                                                                                                                                                          } ?>" ><br><br>


                </div>
                <div class="register_content text-center"><i class="glyphicon glyphicon-envelope"></i></span>
                  <input type="text" name="State" id="email" placeholder="State" class="input_field search_form_name" maxlength="100" value="<?php if (isset($error)) {
                                                                                                                                                            echo $_POST['state'];
                                                                                                                                                          } ?>" ><br><br>
                ...
                <div class="register_content text-center"> <i class="glyphicon glyphicon-user"></i></span>
                  <input name="phoneNumber" id="username" type="text" placeholder="Phone Number" class="input_field search_form_name" maxlength="50" value="<?php if (isset($error)) {
